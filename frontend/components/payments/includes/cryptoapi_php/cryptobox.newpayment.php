...
function cryptobox_new_payment($paymentID = 0, $payment_details = array(), $box_status = "")
{
    ...
    $user = User::findOne($data['user']);
    $order = Order::findOne($data['order']);

    if ($user != null && $order != null) {
        if ($data['status'] == 'payment_received' || $data['status'] == 'payment_received_unrecognised') {
            //Create new transaction
            $trans = new Transaction();
            $trans->setScenario(Transaction::SCENARIO_CREATE);
            $trans->user_id = $order->user_id;
            $trans->amount = $data['amount'];
            $trans->currency = Transaction::CURRENCY_BTC;
            ...
            if (!$trans->save()) {
                \Yii::error("Failed to save transaction. Response from gourl.io is {$dataString}");
            }
        }
        ...
        switch ($data['status']) {
            case 'payment_received':
                ...
                break;
            case 'payment_received_unrecognised':
        ...
        if ($order->save()) {
            ...
            if ($order->status == Order::STATUS_COMPLETED) {
                ...
                if ($voucherId != null) {
                    ...
                    if ($planUpdated) {
                        ...
                        \Yii::error("Failed to update user's plan. Response from gourl.io is {$dataString}");
                    }
                }
            }
            ...
            \Yii::error("Failed to update order. Response from gourl.io is {$dataString}");
        }
    } else {
        \Yii::error("User or order not found. User id is {$data['user']}. Order id is {$data['order']}. Response from gourl.io is {$dataString}");
    }

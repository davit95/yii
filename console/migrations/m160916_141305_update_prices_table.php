...
class m160916_141305_update_prices_table extends Migration
{
    public function up()
    {
        ...
        foreach ($queries->each() as $id => $value) {
            \Yii::$app->db->createCommand("UPDATE `prices` SET `product_name`= '".$product_names[$id]."' ,`description`= '".$descriptions[$id]."' WHERE id = ".$id)->execute();
        }
    }

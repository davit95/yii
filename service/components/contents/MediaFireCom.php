...
/**
 * mediafire.com content
 */
class MediaFireCom extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $form = $domDoc->getElementById('form_login1');
            if ($form === null) {
                return null;
            }
            ...
            foreach ($form->getElementsByTagName('input') as $input) {
                if ($input->getAttribute('name') == 'security') {
                    $security = $input->getAttribute('value');
                }
            }
            ...
            $reqBody = $req->getBody();
            $reqBody->setField('login_email', $credential->getDecryptedUser());
            $reqBody->setField('login_pass', $credential->getDecryptedPass());
            $reqBody->setField('security', $security);
            $reqBody->setField('login_remember', 'off');

...
/**
 * speedyshare.com content
 */
class SpeedyShareCom extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getDownloadLink($url)
    {
        ...
        if ($links != null) {
            for ($i = 0; $i < $links->length; $i++) {
                ...
                if (preg_match('/\/[\w]+\/[\w]+\/download\/.+/', $href)) {
                    if (strpos($href, 'http://') === false) {
                        $href = 'http://www.speedyshare.com'.$href;
                    }
                    return $href;
                }
            }

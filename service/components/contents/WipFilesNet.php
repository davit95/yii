...
/**
 * wipfiles.net content
 */
class WipFilesNet extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $reqBody->setField('redirect', 'http://wipfiles.net/');
    ...
    public function getDownloadLink($url)
    {
        ...
        preg_match("/\/([^\/]+)\.html/", $url, $res);
        $id = isset($res[1]) ? $res[1] : '';

        $reqBody = $req->getBody();
        $reqBody->setField('id', $id);
        $reqBody->setField('method_premium', '1');
        $reqBody->setField('op', 'download2');
        $reqBody->setField('rand', '');
        $reqBody->setField('referer', 'http://wipfiles.net/?op=my_files');

        $resp = $client->send($req);

        $body = (string)$resp->getBody();

        preg_match("/href=\"(http:\/\/.+wipfiles\.net:.+)\"/U", $body, $res);

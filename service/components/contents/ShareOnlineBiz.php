...
/**
 * share-online.biz content
 */
class ShareOnlineBiz extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $reqBody = $req->getBody();
            $reqBody->setField('user', $credential->getDecryptedUser());
    ...
    public function getDownloadLink($url)
    {
        ...
        //Get a cookie. This is required to download
        $aCookie = null;
        $cookies = $resp->getHeaderAsArray('Set-Cookie');
        foreach ($cookies as $cookie) {
            if (preg_match('/^a/', $cookie)) {
                $aCookie = SetCookie::fromString($cookie);
            }
        }

        if ($aCookie == null) {
            //No a cookie, something is wrong
            throw new Exception('Form for generating download URL not found on page.');
        }

        $authCookies = explode('; ', $this->authCookies);
        $authCookies[] = $aCookie->getName() . '=' . CookieJar::getCookieValue($aCookie->getValue());
        $this->authCookies = implode('; ', $authCookies);

        if (!empty($link = $resp->getHeader('location'))) {
            if (preg_match('/http:\/\/dlw\d+-\d+\.share-online\.biz\/.+/', $link)) {
                //Direct download is on, return link
                return $link;
            }
        }
        ...
        //Link is in base64 encoded string, so we need to find that string and decode it to get link
        if (preg_match('/dl=\"([A-Za-z0-9\+\=\/]+)\"/', $body, $matches)) {

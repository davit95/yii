...
/**
 * filefactory.com content
 */
class FileFactoryCom extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $reqBody = $req->getBody();
            $reqBody->setField('loginEmail', $credential->getDecryptedUser());
            $reqBody->setField('loginPassword', $credential->getDecryptedPass());
            $reqBody->setField('Submit', 'Sign In');
    ...
    public function getDownloadLink($url)
    {
        ...
        preg_match("/href=\"(http:\/\/s[\d]+.+)\"/U", $body, $res);

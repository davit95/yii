...
/**
 * interfile.net content
 */
class InterfileNet extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $req->setHeader('Cookie', 'lang=english;');
            $reqBody = $req->getBody();
            ...
            if ($values) {
                $this->authCookies = 'lang=english; ' . implode('; ', $values);
            } else {
    ...
    public function getDownloadLink($url)
    {
        ...
        preg_match("/href=\"(http:\/\/s[\d]+.+)\">/", $body, $res);

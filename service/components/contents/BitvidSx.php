...
/**
 * bitvid.sx (videoweed.es) content
 */
class BitvidSx extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getDownloadLink($url)
    {
        ...
        if (!preg_match('/download\.php\?[^\"]+/', $body, $matches)) {
            throw new Exception('Download URL not found on page.');
        }

        $url = 'http://www.bitvid.sx/'.$matches[0];
        ...
        if (!preg_match('/http:\/\/s\d+\.zerocdn\.to\/.+/', $href)) {
            throw new Exception('Download URL not found on page.');
        }

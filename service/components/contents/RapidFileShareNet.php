...
/**
 * rapidfileshare.net content
 */
class RapidFileShareNet extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getDownloadLink($url)
    {
        ...
        do {
            $req = $client->createRequest('POST', $url, ['verify' => false]);
            $req->setHeader('User-Agent', $this->getUserAgent());
            $req->setHeader('Cookie', $this->getAuthCookies());
            $reqBody = $req->getBody();
        ...
        if ($links != null) {
            for ($i = 0; $i < $links->length; $i++) {
                ...
                if (preg_match('/http:\/\/trinity\.rapidfileshare\.net(:\d+)?\/.+/', $href)) {
                    return $href;
                }
            }
        }

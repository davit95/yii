...
/**
 * clicknupload.link content
 */
class ClicknUploadLink extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getDownloadLink($url)
    {
         ...
         */
        preg_match('/http:\/\/clicknupload\.link\/(.+)/', $url, $matches);
        ...
        $reqBody->setField('op', 'download1');
        $reqBody->setField('id', $matches[1]);
        $reqBody->setField('method_free', 'Free Download');
        ...
        do {
            ...
            if (!$formExists) {
                $els = $domDoc->getElementsByTagName('button');
            }

            $fails++;
        } while ($els == null && $fails <= 2);

        if ($els != null) {
            for ($i = 0; $i < $els->length; $i++) {
                $onclick = $els->item($i)->getAttribute('onclick');
                if (preg_match('/http:\/\/fs\d+\.clicknupload\.[a-z]{2,3}(:\d+)?\/[^\"\']+/', $onclick, $matches)) {
                    return $matches[0];
                }
            }
        }

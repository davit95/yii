 ...
 * uplea.com content
 */
class UpleaCom extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getAuthCookies()
    {
        if ($this->authCookies === null) {
            ...
            $reqBody->setField('remember', '1');
            $reqBody->setField('login-form', '');
    ...
    public function getDownloadLink($url)
    {
        ...
        if ($this->link->hasPassword()) {
            ...
            $reqBody = $req->getBody();
            $reqBody->setField('my-password', $this->link->getPassword());
        } else {
        ...
        if (preg_match('/http:\/\/.+\.uplea\.com\/[^"]+/', $body, $matches)) {
            return $matches[0];
        }

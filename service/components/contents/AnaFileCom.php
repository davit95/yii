 ...
 * anafile.com content
 */
class AnaFileCom extends HttpContent
{
    private $proxy;
    private $userAgent;
    private $authCookies;
    ...
    public function getDownloadLink($url)
    {
        ...
        $fileCookies = array();
        foreach ($resp->getHeaderAsArray('Set-Cookie') as $cookie) {
            $fileCookies[] = SetCookie::fromString($cookie);
        }

        $reqCookies = explode('; ', $req->getHeader('Cookie'));

        foreach ($fileCookies as $cookie) {
            $reqCookies[] = $cookie->getName() . '=' . CookieJar::getCookieValue($cookie->getValue());
        }

        $reqCookies = implode('; ', $reqCookies);
        ...
        if ($links != null) {
            for ($i = 0; $i < $links->length; $i++) {
                $attr = $links->item($i)->attributes->getNamedItem('href');
                if ($attr != null) {
                    $href = $attr->nodeValue;
                    if (preg_match('/http:\/\/anaflz\.net(:\d+)?\/.+/', $href)) {
                        return $href;
                    }
                }
            }
